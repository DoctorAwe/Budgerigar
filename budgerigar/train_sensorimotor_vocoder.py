...
def train_sensorimotor_vocoder(manifest,output_dir,training=SensorimotorTrainingConfig(),model_config=SensorimotorVocoderConfig()):
    ...
    for epoch in range(training.epochs):
        ...
        for ticks,_,_,_ in train_loader:
            ticks=ticks.to(device);waveform=ticks.flatten(1);reconstructed,diagnostics=model(ticks);reconstructed=reconstructed.flatten(1);wave=functional.l1_loss(reconstructed,waveform);spectral=_spectral_loss(torch,reconstructed.float(),waveform.float());source=_source_loss(torch,functional,diagnostics["source_parameters"],waveform,model_config);source_controls=diagnostics["source_controls"];articulation=diagnostics["articulation_controls"];smooth=(articulation[:,1:]-articulation[:,:-1]).abs().mean();source_swap=model.render(source_controls.roll(1,0),articulation)[0].flatten(1);articulation_swap=model.render(source_controls,articulation.roll(1,0))[0].flatten(1);correct_error=(reconstructed-waveform).abs().mean(-1);source_error=(source_swap-waveform).abs().mean(-1);articulation_error=(articulation_swap-waveform).abs().mean(-1);contrastive=(training.control_margin+correct_error-source_error).relu().mean()+(training.control_margin+correct_error-articulation_error).relu().mean();loss=wave+spectral+training.source_supervision_weight*source+training.smoothness_weight*smooth+training.control_contrastive_weight*contrastive
