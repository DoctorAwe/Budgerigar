...
def _content_logits(torch,model,articulation,metadata):
    pooled=[]
    for row,item in enumerate(metadata):pooled.append(articulation[row,item["audio_start_tick"]:item["audio_end_tick"]+1].mean(0))
    return model.content_head(torch.stack(pooled))
...
def train_sensorimotor_vocoder(manifest,output_dir,training=SensorimotorTrainingConfig(),model_config=SensorimotorVocoderConfig()):
    ...
    for epoch in range(training.epochs):
        ...
        for ticks,_,_,metadata in train_loader:
            ticks=ticks.to(device);waveform=ticks.flatten(1);labels=torch.tensor([item["label"] for item in metadata],device=device);predicted_source,articulation,_=model.analyze(ticks);oracle_source=_oracle_source_controls(torch,waveform,model_config);reconstructed=model.render(oracle_source,articulation)[0].flatten(1);wave=functional.l1_loss(reconstructed,waveform);spectral=_spectral_loss(torch,reconstructed.float(),waveform.float());source=_source_loss(torch,functional,model.source_parameters(predicted_source),waveform,model_config);content=functional.cross_entropy(_content_logits(torch,model,articulation,metadata),labels);_,heard_reconstruction,_=model.analyze(reconstructed.view_as(ticks));reconstructed_content=functional.cross_entropy(_content_logits(torch,model,heard_reconstruction,metadata),labels);smooth=(articulation[:,1:]-articulation[:,:-1]).abs().mean();source_swap=model.render(oracle_source.roll(1,0),articulation)[0].flatten(1);articulation_swap=model.render(oracle_source,articulation.roll(1,0))[0].flatten(1);correct_error=(reconstructed-waveform).abs().mean(-1);source_error=(source_swap-waveform).abs().mean(-1);articulation_error=(articulation_swap-waveform).abs().mean(-1);contrastive=(training.control_margin+correct_error-source_error).relu().mean()+(training.control_margin+correct_error-articulation_error).relu().mean();loss=wave+spectral+training.source_supervision_weight*source+training.smoothness_weight*smooth+training.control_contrastive_weight*contrastive+training.content_weight*content+training.reconstructed_content_weight*reconstructed_content
            ...
            if step==1 or step%10==0:print(f"[sensorimotor M0] step={step} loss={float(loss):.4f} spectral={float(spectral):.4f} content={float(content):.4f} output_content={float(reconstructed_content):.4f} control_margin={float(contrastive):.4f}",flush=True)
        ...
        with torch.no_grad():
            for ticks,_,_,metadata in validation_loader:
                ...
                labels=torch.tensor([item["label"] for item in metadata],device=device);real_logits=_content_logits(torch,model,articulation,metadata);_,output_articulation,_=model.analyze(reconstructed);output_logits=_content_logits(torch,model,output_articulation,metadata);real_content_correct+=int((real_logits.argmax(-1)==labels).sum());output_content_correct+=int((output_logits.argmax(-1)==labels).sum());content_examples+=len(labels);input_embedding=_spectral_embedding(torch,functional,waveform);output_embedding=_spectral_embedding(torch,functional,reconstructed.flatten(1));different=labels.ne(labels.roll(1));input_separation.extend((input_embedding-input_embedding.roll(1,0)).abs().mean(-1)[different].tolist());output_separation.extend((output_embedding-output_embedding.roll(1,0)).abs().mean(-1)[different].tolist())
        ...
        correct_loss=sum(correct)/len(correct);source_loss=sum(source_shuffled)/len(source_shuffled);articulation_loss=sum(articulation_shuffled)/len(articulation_shuffled);mean_loss=sum(mean_control)/len(mean_control);input_distance=sum(input_separation)/max(len(input_separation),1);output_distance=sum(output_separation)/max(len(output_separation),1);metrics={"epoch":epoch+1,"step":step,"train_loss":total/max(count,1),"validation_spectral_loss":correct_loss,"validation_source_shuffled_spectral_loss":source_loss,"validation_articulation_shuffled_spectral_loss":articulation_loss,"validation_mean_control_spectral_loss":mean_loss,"validation_source_relative_degradation":source_loss/correct_loss-1,"validation_articulation_relative_degradation":articulation_loss/correct_loss-1,"validation_mean_relative_degradation":mean_loss/correct_loss-1,"validation_real_digit_accuracy":real_content_correct/max(content_examples,1),"validation_output_digit_accuracy":output_content_correct/max(content_examples,1),"validation_input_separation":input_distance,"validation_output_separation":output_distance,"validation_output_separation_ratio":output_distance/max(input_distance,1e-6)};history.append(metrics);print(json.dumps(metrics),flush=True);checkpoint={"architecture":"sensorimotor_content_constrained_m0","model":model.state_dict(),"model_config":asdict(model_config),"training_config":asdict(training),"history":history};torch.save(checkpoint,output_dir/"last.pt");torch.save(example,output_dir/"validation_example.pt");selection=correct_loss+.5*(1-metrics["validation_output_digit_accuracy"])+max(.1-metrics["validation_articulation_relative_degradation"],0)
        if selection<best:best=selection;torch.save(checkpoint,output_dir/"best.pt")
        if step>=training.max_steps:break
    report={"architecture":"sensorimotor_content_constrained_m0","steps":step,"best_selection_score":best,"history":history};(output_dir/"training_report.json").write_text(json.dumps(report,indent=2),encoding="utf-8");return report
