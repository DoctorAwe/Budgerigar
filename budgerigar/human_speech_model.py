...
def create_human_speech_model(config=HumanSpeechConfig()):
    ...
    class HumanSpeechModel(nn.Module):
        ...
        def synthesize(self,latent,state=None,phase=None):
            ...
            motor,motor_state=self.motor(self.motor_projection(latent),motor_state);controls=self._controls(motor);envelope=(2*self.vocal_tract(motor).tanh()).exp();positions=torch.arange(config.synthesis_frame_samples,device=latent.device,dtype=latent.dtype).view(1,1,-1);omega=2*torch.pi*controls["f0_hz"]/config.sample_rate;starts=pitch_phase.unsqueeze(-1)+torch.cat([latent.new_zeros(batch,1),torch.cumsum(omega[:,:-1]*hop,-1)],-1);angles=starts.unsqueeze(-1)+omega.unsqueeze(-1)*positions;harmonic=(torch.sin(angles)+.35*torch.sin(2*angles)+.15*torch.sin(3*angles))/1.5;absolute=torch.arange(sample_offset,sample_offset+steps*hop,hop,device=latent.device,dtype=latent.dtype).view(1,steps,1)+positions;hashed=torch.sin(absolute*12.9898)*43758.5453;noise=(hashed-torch.floor(hashed))*2-1;voiced=controls["voiced"].unsqueeze(-1);aperiodic=controls["aperiodicity"].unsqueeze(-1);excitation=voiced*harmonic+(1-voiced+.25*aperiodic)*noise;spectrum=torch.fft.rfft(excitation.float(),n=config.synthesis_frame_samples)*envelope.float();frames=torch.fft.irfft(spectrum,n=config.synthesis_frame_samples).to(latent.dtype)*self.synthesis_window.to(latent);frames=frames*(2*controls["pressure"].unsqueeze(-1));outputs=[]
