...
def _control_loss(torch,functional,controls,waveform,config):
    energy,pitch,voiced=_source_targets(torch,functional,waveform,config);pitch_error=functional.l1_loss((controls["f0_hz"]-60)/340,pitch,reduction="none");pitch_error=(pitch_error*voiced).sum()/voiced.sum().clamp_min(1);return functional.l1_loss(controls["pressure"],energy)+pitch_error+functional.binary_cross_entropy(controls["voiced"],voiced)
...
def train_human_speech(manifest,output_dir,evaluator_checkpoint,training=HumanSpeechTrainingConfig(),model_config=HumanSpeechConfig()):
    ...
    for epoch in range(training.epochs):
        ...
        correct_loss=sum(correct)/len(correct);shuffled_loss=sum(shuffled)/len(shuffled);mean_loss=sum(mean_values)/len(mean_values);input_distance=sum(input_separation)/max(len(input_separation),1);output_distance=sum(output_separation)/max(len(output_separation),1);metrics={"epoch":epoch+1,"step":step,"train_loss":total/max(count,1),"validation_acoustic_loss":correct_loss,"validation_shuffled_sensation_loss":shuffled_loss,"validation_mean_sensation_loss":mean_loss,"validation_shuffled_relative_degradation":shuffled_loss/correct_loss-1,"validation_mean_relative_degradation":mean_loss/correct_loss-1,"validation_energy_envelope_l1":sum(energy_errors)/len(energy_errors),"validation_voiced_duration_ratio":sum(duration_ratios)/len(duration_ratios),"validation_frozen_real_digit_accuracy":real_correct/max(examples,1),"validation_frozen_output_digit_accuracy":output_correct/max(examples,1),"validation_output_separation_ratio":output_distance/max(input_distance,1e-6)};history.append(metrics);print(json.dumps(metrics),flush=True);checkpoint={"architecture":"human_cochlear_shared_motor_synthesis_m0","model":model.state_dict(),"optimizer":optimizer.state_dict(),"model_config":asdict(model_config),"training_config":asdict(training),"history":history};torch.save(checkpoint,output_dir/"last.pt");torch.save(example,output_dir/"validation_example.pt");selection=correct_loss+.5*(1-metrics["validation_frozen_output_digit_accuracy"])
        if selection<best:best=selection;torch.save(checkpoint,output_dir/"best.pt")
        if step>=training.max_steps:break
    report={"architecture":"human_cochlear_shared_motor_synthesis_m0","steps":step,"best_selection_score":best,"history":history};(output_dir/"training_report.json").write_text(json.dumps(report,indent=2),encoding="utf-8");return report
