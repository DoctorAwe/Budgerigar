...
def audit_short_memory(manifest,checkpoint_path,output_path=None,split="validation",batch_size=16,max_records=300,seed=97):
    ...
    reconstruction={name:value/max(reconstruction_examples,1) for name,value in reconstruction.items()};report={"architecture":checkpoint.get("architecture"),"checkpoint":str(checkpoint_path),"split":split,"examples":total,"model_config":asdict(config),"prefix_digit_accuracy":{str(int(value*100)):prefix[value]/total for value in fractions},"silence_retention_accuracy":{str(value):delay[value]/total for value in delays_ms},"memory_ablation_accuracy":{"intact":end_correct/total,"all_cleared":cleared_correct/total,"batch_shuffled":shuffled_correct/total,"layer_cleared":{str(index+1):value/total for index,value in enumerate(layer_correct)}},"input_ablation_accuracy":{"full_input_zeroed":full_zero_correct/total,"last_ms_zeroed":{str(value):tail_correct[value]/total for value in tail_ms}},"reconstruction_l1":reconstruction,"audit_pass":end_correct/total>.8 and delay[1000]/total>.7 and cleared_correct/total<.2 and shuffled_correct/total<.2 and reconstruction["correct"]<reconstruction["shuffled"]}
