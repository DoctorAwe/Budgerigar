...
from .train_short_memory import acoustic_reconstruction_targets
...
def audit_short_memory(manifest,checkpoint_path,output_path=None,split="validation",batch_size=16,max_records=300,seed=97):
    ...
    fractions=(.25,.5,.75,1.0);delays_ms=(0,100,200,300,500,1000);tail_ms=(100,200);prefix={value:0 for value in fractions};delay={value:0 for value in delays_ms};layer_correct=[0]*config.token_layers;total=end_correct=cleared_correct=shuffled_correct=full_zero_correct=acoustic_cleared_correct=0;tail_correct={value:0 for value in tail_ms};reconstruction={"correct":0.,"shuffled":0.,"mean_template":0.};reconstruction_examples=0
    with torch.no_grad():
        for samples,_,_,metadata in loader:
            ...
            tokens=_end_tokens(torch,diagnostics["token_history"],metadata,device);acoustic=_end_tokens(torch,diagnostics["acoustic_history"],metadata,device) if model.use_acoustic_memory else None;end_logits,reconstructed,_=model.decode_memory(tokens,acoustic);end_correct+=_accuracy(end_logits,labels)
            initial=model.initial_tokens.unsqueeze(0).expand_as(tokens);cleared_correct+=_accuracy(model.decode_memory(initial,acoustic)[0],labels);shuffled_correct+=_accuracy(model.decode_memory(tokens.roll(1,0),acoustic)[0],labels)
            if model.use_acoustic_memory:
                initial_acoustic=model.initial_acoustic.unsqueeze(0).expand_as(acoustic);acoustic_cleared_correct+=_accuracy(model.decode_memory(tokens,initial_acoustic)[0],labels)
            for layer in range(config.token_layers):
                ablated=tokens.clone();ablated[:,layer]=initial[:,layer];layer_correct[layer]+=_accuracy(model.decode_memory(ablated,acoustic)[0],labels)
            if model.use_acoustic_memory:targets=acoustic_reconstruction_targets(torch,functional,samples,metadata,config)
            else:
                target_rows=[]
                for row,meta in enumerate(metadata):heard=diagnostics["encoded_features"][row,:meta["audio_end_tick"]+1].transpose(0,1).unsqueeze(0);target_rows.append(functional.adaptive_avg_pool1d(heard,config.reconstruction_slots).squeeze(0).transpose(0,1))
                targets=torch.stack(target_rows)
            mean_target=targets.mean(0,keepdim=True).expand_as(targets);reconstruction["correct"]+=float(functional.l1_loss(reconstructed,targets,reduction="sum"));reconstruction["shuffled"]+=float(functional.l1_loss(reconstructed,targets.roll(1,0),reduction="sum"));reconstruction["mean_template"]+=float(functional.l1_loss(reconstructed,mean_target,reduction="sum"));reconstruction_examples+=targets.numel()
