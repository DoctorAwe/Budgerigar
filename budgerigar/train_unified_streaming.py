...
def train_unified_streaming(manifest,output_dir,evaluator_checkpoint,training=UnifiedStreamingTrainingConfig(),model_config=UnifiedStreamingConfig()):
    ...
    for epoch in range(training.epochs):
        ...
        for ticks,_,_,metadata in train_loader:
            ticks=ticks.to(device);waveform=ticks.flatten(1);labels=torch.tensor([item["label"] for item in metadata],device=device);reconstructed,_,diagnostics=model(ticks);reconstructed=reconstructed.flatten(1);wave=functional.l1_loss(reconstructed,waveform);spectral=_spectral_loss(torch,reconstructed.float(),waveform.float());probe=_source_probe_loss(torch,functional,diagnostics["source_probe"],waveform,model_config);content=functional.cross_entropy(evaluator(robust_waveform_augmentation(torch,reconstructed)),labels);loss=wave+spectral+training.source_probe_weight*probe+training.frozen_content_weight*content;optimizer.zero_grad(set_to_none=True);loss.backward();torch.nn.utils.clip_grad_norm_(model.parameters(),1.0);optimizer.step();step+=1;total+=float(loss.detach());count+=1
            if step==1 or step%10==0:print(f"[unified M0] step={step} loss={float(loss.detach()):.4f} spectral={float(spectral.detach()):.4f} frozen_content={float(content.detach()):.4f} source_probe={float(probe.detach()):.4f}",flush=True)
