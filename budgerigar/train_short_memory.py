...
class ShortMemoryTrainingConfig:
    ...
    acoustic_contrastive_weight:float=1.0
...
def acoustic_reconstruction_targets(torch,functional,samples,metadata,model_config):
    window=torch.hann_window(256,device=samples.device,dtype=samples.dtype);targets=[]
    for row,meta in enumerate(metadata):
        waveform=samples[row,:meta["audio_end_tick"]+1].flatten();spectrum=torch.stft(waveform,n_fft=256,hop_length=80,win_length=256,window=window,center=False,return_complex=True).abs().log1p();pooled=functional.interpolate(spectrum.unsqueeze(0).unsqueeze(0),size=(model_config.acoustic_bins,model_config.acoustic_reconstruction_slots),mode="bilinear",align_corners=False);targets.append(pooled[0,0].transpose(0,1))
    return torch.stack(targets)

def _memory_losses(torch,functional,model,diagnostics,metadata,labels,samples):
    ...
    end_tokens=diagnostics["token_history"][rows,end_index];delay_tokens=diagnostics["token_history"][rows,delay_index];end_acoustic=diagnostics["acoustic_history"][rows,end_index] if model.use_acoustic_memory else None;delay_acoustic=diagnostics["acoustic_history"][rows,delay_index] if model.use_acoustic_memory else None;end_content,end_reconstruction,_=model.decode_memory(end_tokens,end_acoustic);delay_content,_,_=model.decode_memory(delay_tokens,delay_acoustic)
    ...
    if model.use_acoustic_memory:target=acoustic_reconstruction_targets(torch,functional,samples,metadata,model.config).detach()
    else:
        targets=[]
        for row,meta in enumerate(metadata):
    ...
    reconstruction_loss=functional.smooth_l1_loss(end_reconstruction,target);correct=(end_reconstruction-target).abs().mean((1,2));shuffled=(end_reconstruction-target.roll(1,0)).abs().mean((1,2));mean=(end_reconstruction-target.mean(0,keepdim=True)).abs().mean((1,2));contrastive=(.1+correct-shuffled).relu().mean()+(.05+correct-mean).relu().mean()
    return content_loss,reconstruction_loss,retention_loss,contrastive

def train_short_memory(manifest,output_dir,training=ShortMemoryTrainingConfig(),model_config=ShortMemoryConfig(),initialize_from=None):
    ...
    if initialize_from is not None:
        source=torch.load(initialize_from,map_location=device,weights_only=False);current=model.state_dict();compatible={name:value for name,value in source["model"].items() if name in current and current[name].shape==value.shape};missing=model.load_state_dict(compatible,strict=False);print(f"[memory init] loaded {len(compatible)} tensors from {initialize_from}; new={len(missing.missing_keys)}",flush=True)
    ...
    for epoch in range(training.epochs):
        ...
        for samples,_,valid,metadata in train_loader:
            ...
            content_loss,reconstruction_loss,retention_loss,acoustic_contrastive=_memory_losses(torch,functional,model,diagnostics,metadata,labels,samples);ctc=_window_ctc(torch,functional,logits,metadata,device);emission_loss,peak=_window_emission_loss(torch,functional,diagnostics["emission_probability"],metadata);timing_loss,early_peak,_=_relative_timing_loss(torch,diagnostics["emission_probability"],metadata,training.timing_margin)
            blank_mask=(early|late)&valid;blank_loss=functional.cross_entropy(logits[blank_mask],torch.zeros(int(blank_mask.sum()),dtype=torch.long,device=device));loss=training.ctc_weight*ctc+training.content_weight*content_loss+training.reconstruction_weight*reconstruction_loss+training.acoustic_contrastive_weight*acoustic_contrastive+training.retention_weight*retention_loss+training.blank_weight*blank_loss+training.emission_weight*emission_loss+training.timing_weight*timing_loss
            ...
            if step==1 or step%10==0:print(f"[dual memory] step={step} loss={float(loss):.4f} content={float(content_loss):.4f} reconstruct={float(reconstruction_loss):.4f} acoustic_nce={float(acoustic_contrastive):.4f} retain={float(retention_loss):.4f} timing={float(timing_loss):.4f}",flush=True)
            if step>=training.max_steps:break
        model.eval();end_correct=memory_correct=retained=emission_correct=emitted=examples=early_count=within=0;latencies=[];stream_match=[];window_peaks=[];prewindow_peaks=[];reconstruction_errors=[];reconstruction_shuffled=[];reconstruction_mean=[]
        with torch.no_grad():
            for samples,_,valid,metadata in validation_loader:
                ...
                rows=torch.arange(len(metadata),device=device);end_indices=torch.tensor([meta["audio_end_tick"] for meta in metadata],device=device);end_tokens=diagnostics["token_history"][rows,end_indices];end_acoustic=diagnostics["acoustic_history"][rows,end_indices] if model.use_acoustic_memory else None;_,reconstructed,_=model.decode_memory(end_tokens,end_acoustic)
                if model.use_acoustic_memory:targets=acoustic_reconstruction_targets(torch,functional,samples,metadata,model_config)
                else:
                    target_rows=[]
                    for row,meta in enumerate(metadata):heard=diagnostics["encoded_features"][row,:meta["audio_end_tick"]+1].transpose(0,1).unsqueeze(0);target_rows.append(functional.adaptive_avg_pool1d(heard,model_config.reconstruction_slots).squeeze(0).transpose(0,1))
                    targets=torch.stack(target_rows)
                reconstruction_errors.extend((reconstructed-targets).abs().mean((1,2)).tolist());reconstruction_shuffled.extend((reconstructed-targets.roll(1,0)).abs().mean((1,2)).tolist());reconstruction_mean.extend((reconstructed-targets.mean(0,keepdim=True)).abs().mean((1,2)).tolist())
        metrics={"epoch":epoch+1,"step":step,"train_loss":total/max(count,1),"validation_end_digit_accuracy":end_correct/max(examples,1),"validation_memory_digit_accuracy":memory_correct/max(examples,1),"validation_memory_retention_rate":retained/max(examples,1),"validation_reconstruction_l1":sum(reconstruction_errors)/max(len(reconstruction_errors),1),"validation_reconstruction_shuffled_l1":sum(reconstruction_shuffled)/max(len(reconstruction_shuffled),1),"validation_reconstruction_mean_l1":sum(reconstruction_mean)/max(len(reconstruction_mean),1),"validation_window_emission_accuracy":emission_correct/max(examples,1),"validation_emission_recall":emitted/max(examples,1),"validation_early_emission_rate":early_count/max(examples,1),"validation_mean_latency_ms":sum(latencies)/max(len(latencies),1),"validation_prewindow_peak_probability":sum(prewindow_peaks)/len(prewindow_peaks),"validation_window_peak_probability":sum(window_peaks)/len(window_peaks),"streaming_max_logit_difference":max(stream_match)};history.append(metrics);print(json.dumps(metrics),flush=True)
        architecture="streaming_cochlear_dual_semantic_acoustic_memory" if model.use_acoustic_memory else "streaming_cochlear_hierarchical_memory_decoder";score=metrics["validation_memory_digit_accuracy"]+metrics["validation_end_digit_accuracy"]-metrics["validation_early_emission_rate"]+metrics["validation_reconstruction_mean_l1"]-metrics["validation_reconstruction_l1"];checkpoint={"architecture":architecture,"model":model.state_dict(),"model_config":asdict(model_config),"training_config":asdict(training),"history":history};torch.save(checkpoint,output_dir/"last.pt")
        if score>best:best=score;torch.save(checkpoint,output_dir/"best.pt")
        if step>=training.max_steps:break
    report={"architecture":"streaming_cochlear_dual_semantic_acoustic_memory" if model.use_acoustic_memory else "streaming_cochlear_hierarchical_memory_decoder","steps":step,"best_score":best,"history":history};(output_dir/"training_report.json").write_text(json.dumps(report,indent=2),encoding="utf-8");return report
