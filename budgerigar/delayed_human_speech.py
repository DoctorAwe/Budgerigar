...
def create_delayed_human_speech(config=DelayedHumanSpeechConfig()):
    ...
    class DelayedHumanSpeech(nn.Module):
        ...
        def forward(self,ticks,state=None):
            ...
            sensation,auditory_state=self.human.encode(ticks,auditory_state);batch,length,_=ticks.shape;evidence=self.sensation_adapter(sensation.reshape(batch,length,-1));tokens=self.semantic.initial_tokens.unsqueeze(0).expand(batch,-1,-1).clone() if tokens is None else tokens;reads=[];token_history=[];content=[]
            ...
            reads=torch.stack(reads,1);recall=self.recall_projection(reads).view(batch,length*config.human.subframes,config.human.latent_dim);waveform,motor_state,phase,diagnostics=self.human.synthesize(recall,motor_state,phase);diagnostics.update({"sensation":sensation,"semantic_evidence":evidence,"token_states":tokens,"token_history":torch.stack(token_history,1),"content_logits":torch.stack(content,1),"recall":recall});return waveform,(auditory_state,tokens,motor_state,phase),diagnostics
